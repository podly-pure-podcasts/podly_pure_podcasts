...
class AudioProcessor:
    ...
    def get_ad_segments(self, post: Post) -> list[tuple[float, float]]:
        ...
        if getattr(self.config, "enable_boundary_refinement", False):
            # Check DB and object for refined boundaries
            post_row = self._safe_get_post_row(post)
            refined = (
                getattr(post_row, "refined_ad_boundaries", None) if post_row else None
            )
            if not refined:
                refined = getattr(post, "refined_ad_boundaries", None)

            parsed = self._parse_refined_boundaries(refined)
            if parsed:
                parsed.sort(key=lambda x: x[2])  # Sort by refined_start
                self.logger.info(
                    f"Found {len(parsed)} persisted refined boundaries for post {post.id}."
                )
    ...
    def _parse_refined_boundaries(
        ...
        for item in refined:
            ...
            if refined_start_raw is None or refined_end_raw is None:
                continue

            try:
                refined_start = float(refined_start_raw)
                refined_end = float(refined_end_raw)
                # Fallback to refined if orig missing
                orig_start = float(
                    orig_start_raw if orig_start_raw is not None else refined_start
                )
                orig_end = float(
                    orig_end_raw if orig_end_raw is not None else refined_end
                )
            except Exception:  # noqa: BLE001
                continue
