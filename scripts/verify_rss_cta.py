...
def _verify_single_rss_item(
    item, target_guid: str | None = None, namespaces: dict | None = None
) -> tuple[bool, str | None]:
    """Helper to verify a single RSS item."""
    ...
    content_elem = item.find("content:encoded", namespaces or {})
...
def verify_rss_cta(feed_url: str, target_guid: str | None = None) -> bool:
    ...
    for item in items:
        item_passed, guid = _verify_single_rss_item(
            item, target_guid=target_guid, namespaces=namespaces
        )

        if target_guid and not guid:
            continue
